...
public class MapWalkRoutMainActivity extends AppCompatActivity implements View.OnClickListener, MapEventListener,
...
//    private CustomBalloon customBalloon;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 2:                                // Key 인증 실패처리
                    ...
                    Toast.makeText(MapWalkRoutMainActivity.this, toastMsg, Toast.LENGTH_LONG).show();
                    break;
            }
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Coord startpoint = null;
        Coord transCoord;
        Coord[] lines = new Coord[walk_points.size()];
        for (int k = 0; k < walk_points.size(); k++) {
            Coord coord = new Coord(walk_points.get(k).getX(), walk_points.get(k).getY());

            transCoord = mapView.getProjection().transCoordination(Projection.WGS84,
                    Projection.UTMK, coord);
            lines[k] = transCoord;
            mapView.setMapCenter(transCoord);
//                Coord getcoord = mapView.getMapCenter();
//                Marker marker = new Marker(getcoord, getResources().getDrawable(number[k]));
//                markerLayer.addItem(marker);
            ...
            if (k == 0) {
                ...
                startpoint = getcoord;
            } else if (k == walk_points.size() - 1) {
                //도착마커
                marker = new Marker(getcoord, getResources().getDrawable(R.mipmap.ic_end_marker));
            }
            markerLayer.addItem(marker);
        }
        mapView.setMapCenter(startpoint);

        Polyline polyline = new Polyline(lines, 20, getResources().getColor(R.color.blue_walk));
//        polyline.setLineType(Polyline.DASH_LINE);
    ...
    public void onClick(View v) {
        ...
        switch (id) {
            case R.id.map_rout_myspot:                    // Toggle = OFF면 On시키고 ON이면 OFF합니다.
                gps = new GpsInfo(MapWalkRoutMainActivity.this);
                if (gps.isGetLocation()) {
                    ...
                    // GPS 를 사용할수 없으므로
                    AlertDialog.Builder gsDialog = new AlertDialog.Builder(MapWalkRoutMainActivity.this);
